...
contract LaypipeDividendDistributor is IUnlockCallback, IUniswapV3SwapCallback, ReentrancyGuard {
    ...
    function register(PoolId poolId, address token, DividendParams calldata p) external {
        ...
        try ILaypipeCheckpointToken(token).supportsBlockCheckpoints() returns (bool supported) {
    ...
    function startRound(PoolId poolId) external nonReentrant returns (uint64 roundId) {
        ...
        if (block.number == 0 || block.number - 1 > type(uint64).max) revert SnapshotNotReady();
        uint64 eligibilityCheckpoint = uint64(block.number - 1);
        if (ILaypipeCheckpointToken(cfg.token).totalSupplyAt(eligibilityCheckpoint) == 0) {
